...
dei residui e vengono divisi in blocchi 8x8 e viene applicata la trasformata discreta del coseno
(\textbf{DCT} - Discrete Cosine Transform).

La trasformata DCT consiste nel applicare un filtro di dimensioni costante (il codec H.264 utilizza un
filtro di dimensioni 16x16 per le matrici Luma e 4x4 per le matrici Chroma) alle matrici presenti
all'interno di un macroblock, così facendo trasforma i valori dei pixel dal dominio spaziale al
dominio delle frequenze individuando così i blocchi che contengono più informazioni, ovvero quelli
con frequenza bassa.

Cosi facendo, si avranno i blocchi 8x8 ordinati in base al livello della loro frequenza.

\paragraph{Quantization}\hphantom{A}\\
La quantizzazione coinvolge la divisione dei blocchi 8x8 precedentemente calcolati per una costante,
seguita dall'arrotondamento al numero intero più vicino. Questo processo comporta una perdita di
dettaglio, soprattutto nelle aree ad alta frequenza (quindi meno evidenti), ma porta a una riduzione
delle dimensioni del file video. In pratica, maggiore è la costante con cui dividiamo la matrice,
maggiore è il risparmio di memoria, ma al contempo peggiora la qualità dell'immagine.

Organizzare i blocchi in base alla loro frequenza è vantaggioso in quanto facilita una codifica e
decodifica più efficiente. Questa strategia consente di identificare i blocchi più complessi da
elaborare in anticipo.

TODO: potrei mettere un immagine di come si quantizza una matrice
