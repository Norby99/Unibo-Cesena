...
I clienti devono essere in grado di prenotare dei tavoli, ma non più di uno a mezz'ora. Inoltre, non possono esserci più di 10 tavoli prenotati
...
Infine, che i pizzaioli possano monitorare gli eventi della pizzeria, devono
...
L'applicazione è strutturata come un sito web implementato su Apache, con un database SQL ospitato su MySQL, e fa uso del framework CSS Bootstrap. All'apertura del sito, gli utenti si trovano di fronte a una pagina di accesso, la quale presenta diverse home page a seconda del tipo di utente che si è autenticato.

L'utente standard ha la possibilità di creare il proprio ordine, selezionando le pizze da aggiungere al carrello e successivamente potrà confermare l'ordine inserendo eventualmente i dati di consegna. Inoltre, è in grado di prenotare un tavolo attraverso una specifica pagina e di visualizzare un elenco dettagliato degli ordini effettuati in precedenza.

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{images/pagina_conferma_ordine.png}
    \caption{Pagina di inserimento dei dati per la consegna dell'ordine.}
    \label{fig:pagina_conferma_ordine}
\end{figure}

Dall'altra parte, l'utente con privilegi di 'amministratore' ha accesso a una gamma più ampia di funzioni. Questo utente ha la facoltà di modificare le pizze (aggiungendone, apportando modifiche o eliminandole), gestire gli ingredienti e consultare la classifica delle pizze più vendute. Inoltre, può accedere alla cronologia completa di tutti gli ordini effettuati, visualizzando sia quelli già elaborati che quelli in attesa di preparazione. L'amministratore ha anche la possibilità di monitorare i tavoli prenotati per una specifica data e fascia oraria.
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{images/pagina_pizze.png}
    \caption{Pagina principale dove poter visualizzare/modificare/aggiungere nuove pizze.}
    \label{fig:pagina_pizze}
\end{figure}
L'applicazione soddisfa ogni funzionalità descritta nella fase di progettazione.
